...
/**
 * MongoMain class and central entry point of the MongoFlyway command-line tool.
 */
public class MongoMain {
    ...
    static void initLogging(Level level) {
        ...
        LOG = LogFactory.getLog(MongoMain.class);
    }
    ...
    /**
     * Executes this operation on this MongoFlyway instance.
     *
     * @param flyway    The MongoFlyway instance.
     * @param operation The operation to execute.
     */
    private static void executeOperation(MongoFlyway flyway, String operation) {
    ...
    private static void initializeDefaults(Properties properties) {
        properties.put("flyway.mongo.locations", "filesystem:" + new File(getInstallationDir(), "js").getAbsolutePath());
    ...
    private static void printUsage() {
        ...
        LOG.info("-------");
        LOG.info("driver                       : Fully qualified classname of the mongo-java driver");
        LOG.info("uri                          : Mongo URI to use to connect to the database");
        ...
        LOG.info("skipDefaultResolvers         : Skips default resolvers (java, js and Spring-jdbc)");
        LOG.info("mongoMigrationPrefix           : File name prefix for mongo js migrations");
        LOG.info("repeatableMongoMigrationPrefix : File name prefix for repeatable mongo js migrations");
        LOG.info("mongoMigrationSeparator        : File name separator for mongo js migrations");
        LOG.info("mongoMigrationSuffix           : File name suffix for mongo js migrations");
        LOG.info("encoding                     : Encoding of Mongo js migrations");
        ...
        LOG.info("skipDefaultCallbacks         : Skips default callbacks (js)");
        ...
        LOG.info("Example");
        LOG.info("-------");
        LOG.info("flyway -uri=mongodb://localhost/db1 migrate");
        LOG.info("");
        LOG.info("More info at https://flywaydb.org/documentation/commandline");
    }

    /**
     * Loads all the driver jars contained in the drivers folder. (For mongo-java drivers)
     *
    ...
    private static void loadJdbcDrivers() throws IOException {
        ...
        if (files == null) {
            LOG.error("Directory for mongo-java drivers not found: " + driversDir.getAbsolutePath());
            System.exit(1);
        }
    ...
    /**
     * If no Mongo URI has been provided, prompt for it. If you want to avoid the prompt,
    ...
    private static void promptForCredentialsIfMissing(Properties properties) {
        ...
        if (!properties.containsKey("flyway.mongo.uri")) {
            // Mongo URI is not set. We are doomed for failure anyway.
            properties.put("flyway.mongo.uri", console.readLine("Mongo URI: "));
        }
    }
    ...
    private static void dumpConfiguration(Properties properties) {
        ...
        for (Map.Entry<Object, Object> entry : properties.entrySet()) {
            ...
            value = "flyway.mongo.uri".equals(entry.getKey()) ? StringUtils.trimOrPad("", value.length(), '*') : value;
    ...
    private static String getInstallationDir() {
        String path = ClassUtils.getLocationOnDisk(MongoMain.class);
